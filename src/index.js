...
async function onSearch(event) {
  ...
  if(collection.totalHits > newsApiService.per_page) {
    LoadMoreBtnActiv();
    counterActive();
    counter.innerHTML = `We found more ${collection.totalHits - newsApiService.per_page * (newsApiService.page - 1)}  images for you`;
  } 

  if(collection.totalHits < newsApiService.per_page) {
    Notiflix.Notify.warning("Sorry, we found only these few images for your request");
  }
}
